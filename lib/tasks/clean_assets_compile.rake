...
namespace :lumen do
  ...
  task :create_non_digest_assets => :"assets:environment"  do
    manifest_path = Dir.glob(File.join(Rails.public_path, 'assets/.sprockets-manifest-*.json')).first
    ...
    manifest_data['assets'].each do |logical_path, digested_path|
      ...
      if Chill::Application.config.assets.non_digest_named_assets.any? {|testpath| logical_pathname.fnmatch?(testpath, File::FNM_PATHNAME) }
        full_digested_path    = File.join(Rails.public_path, 'assets', digested_path)
        full_nondigested_path = File.join(Rails.public_path, 'assets', logical_path)
